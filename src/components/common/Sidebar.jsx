...
function Sidebar() {
    ...
    return (
        <div className={`sidebar bg-white w-screen sm:w-fit h-full shadow-md flex flex-col z-50 ${isOpen ? "translate-x-full" : "translate-x-0"}`}>
            <header className="Sidebar-header h-20 flex items-center gap-4 p-3 font-medium border-b-2 border-b-gray-200">
                ...
                <button onClick={closeSidebar} className="p-2 sm:hidden block">X</button>
            ...
            <main className="sidebar-content p-3 flex flex-col gap-5">
                {navLinks.map((nav) => (
                    <NavLink
                        ...
                        to={nav.route}
                        className={({ isActive }) =>
                            `flex items-center gap-3 p-3 rounded-sm duration-100 transition-all ease-in-out ${
                                isActive ? "bg-gray-100 shadow-sm border-r-4 border-r-gray-900" : ""
                            }`
                        }
                    >
                        <span>{nav.icon}</span>
                        <span className="lg:flex hidden">{nav.name}</span>
                    </NavLink>
                ))}
            </main>
        </div>
