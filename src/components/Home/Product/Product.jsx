...
const totalPages=Math.ceil(burmeseFood.length/pagePerFood);
    ...
    return (
        <div className={`w-full h-full mt-2 px-2 overflow-y-hidden flex flex-col gap-2 `}>
            ...
            <header className=" flex h-1/12  items-center justify-start gap-5">
                {category.map((category) => (
                    <button className={`px-2 w-28 py-2 rounded-sm shadow cursor-pointer 
            ...
            <main className={`h-9/12 w-full   mt-2  gap-2  place-items-start   grid grid-cols-3 grid-rows-3`}>
            ...
            <div className={` h-1/12 paginate Control mt-2  flex self-stretch  items-center justify-self-end gap-5  `}>
